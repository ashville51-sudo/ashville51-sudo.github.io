...
| CR-020 важные страницы заблокированы | пагинация закрыта в robots.txt | `/catalog/page-4/` (`Disallow`) | 1 находка — **после привязки домена**; до неё robots.txt не в корне → ничего не заблокировано → `ok` |
| CR-021 параметрические URL не закрыты | UTM-ссылка не закрыта в robots.txt | `/catalog/?utm_source=poligon-test` | 1 находка (срабатывает и без robots.txt); `/catalog/?filter=…` закрыт — норма после домена, `/catalog/?color=sinij` сработает после добавления `color` в `junk_query_params` (демо Settings) |
...
- CR-016 — 3 находки «canonical не self» (у параметрических URL canonical
  ведёт на `/catalog/` — что, строго говоря, корректная настройка,
  кандидат в «ложное срабатывание» для демо этого статуса).

**До привязки кастомного домена** сайт живёт на
`ashville51-sudo.github.io/seo-poligon-test/`, robots.txt лежит не в корне домена
и краулером не виден: CR-020 даёт `ok` (нет заблокированных URL), «заблокированные»
страницы обходятся, `?filter=` не считается закрытым. Полная картина — после домена.

Поисковая индексация (данные для проверок Вебмастера/GSC) появляется с лагом
1–3 недели после регистрации в сервисах.

## Эталон первого прогона (проверено 2026-07-06, до привязки домена)

`seo-monitor demo <URL полигона>` → **22 проблемы**: CR-002 ×2, CR-003 ×1,
CR-004 ×2, CR-005 ×1, CR-006 ×2, CR-007 ×1, CR-008 ×1, CR-010 ×1, CR-014 ×2,
CR-015 ×1, CR-016 ×4, CR-017 ×1, CR-018 ×1, CR-019 ×1, CR-021 ×1; WM-001 — `ok`.
