...
После переезда в корень (проверено 2026-07-06): тоже **22 проблемы**,
но состав другой — CR-016 ×3 вместо ×4 (`?filter=` заблокирован
и не обходится), появилась CR-020 ×1; остальное без изменений.
Повторный прогон: новых 0, подтверждено 22 (дедупликация Issue).
